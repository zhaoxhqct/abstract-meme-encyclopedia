...
- `src/data/memes.ts` 里的梗名称与分类取自早期的单文件原型，**正文、简介、引语、时间与数字均为 AI 生成的示例文本，没有经过事实核查**，仅用于展示界面与交互。
- 其中包含对真实人物（含已故者）的描述与「引语」，这些引语是模型生成的，**不能当作当事人的真实言论引用**。
- 需要对外展示时，请把 `src/data/memes.ts` 换成你自己核实过的内容，或替换为不涉及真实人物的虚构示例数据。
